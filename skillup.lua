...
function get_sets()
    ...
    gs_skill = {
        skillup_table = {
            "Healing", "Geomancy", "Enhancing", "Ninjutsu", "Singing", "Blue",
            "Summoning"
        },
        skillup_type = 'None',
        skillup_spells = T {},
...
function file_unload()
    if user_settings.save_settings then file_write() end
    window:destroy()
    button:destroy()
end
function status_change(new, old)
    ...
    if new == 'Idle' then
        if skilluprun then
            send_command('wait 1.0;input /ma "' ..
                             gs_skill.skillup_spells[gs_skill.skillup_count] ..
                             '" <me>')
        end
...
function go_to_idle_gear() equip(sets.Idle) end
function filtered_action(spell)
    ...
    if gs_skill.bluspellulid[spell.en] then
        if windower.ffxi.get_ability_recasts()[81] and
            windower.ffxi.get_ability_recasts()[81] == 0 then
            cancel_spell()
            send_command('input /ja "' ..
                             res.job_abilities[298][gearswap.language] ..
                             '" <me>')
        elseif windower.ffxi.get_ability_recasts()[254] and
            windower.ffxi.get_ability_recasts()[254] == 0 then
            cancel_spell()
            send_command('input /ja "' ..
                             res.job_abilities[338][gearswap.language] ..
                             '" <me>')
        else
            cancel_spell()
            gs_skill.skillup_count = (gs_skill.skillup_count %
                                         #gs_skill.skillup_spells) + 1
            send_command('input /ma "' ..
                             gs_skill.skillup_spells[gs_skill.skillup_count] ..
                             '" <me>')
        end
        return
    ...
    elseif S {"Avatar's Favor", "Elemental Siphon"}:contains(spell.en) then
        cancel_spell()
        send_command(
            'input /ja "' .. res.job_abilities[90][gearswap.language] ..
                '" <me>')
    end
...
function precast(spell)
    if pet.isvalid and S {"Release", "Full Circle"}:contains(spell.en) then
        if spell.en == "Release" then
            ...
            if (recast > 0) then
                cancel_spell()
                send_command(
                    'wait ' .. tostring(recast + 0.5) .. ';input /ja "' ..
                        res.job_abilities[90][gearswap.language] .. '" <me>')
            end
            return
        else
            return
        end
    elseif not pet.isvalid and
        S {"Release", "Avatar's Favor", "Elemental Siphon"}:contains(spell.en) then
        cancel_spell()
        send_command('input /heal on')
        return
    end
    ...
    if gs_skill.skillup_type == "Singing" then
        if (gs_skillup.test_mode and gs_skillup.test_brd == "String") or
            not gs_skillup.skill['Stringed Instrument Capped'] then
            equip(sets.brd.string_inst)
        elseif (gs_skillup.test_mode and gs_skillup.test_brd == "Wind") or
            not gs_skillup.skill['Wind Instrument Capped'] then
            equip(sets.brd.wind_inst)
        end
    end
    if spell.en == "Avatar's Favor" then
        if (windower.ffxi.get_ability_recasts()[spell.recast_id] > 0) or
            buffactive["Avatar's Favor"] then
            cancel_spell()
            send_command('input /ja "' ..
                             res.job_abilities[90][gearswap.language] ..
                             '" <me>')
            return
        end
    end
    ...
    if gs_skillup.use_trust and party.count == 1 and
        spell_usable(res.spells[931]) then
        if spell.en ~= "Moogle" then
            cancel_spell()
            send_command('input /ma "' .. res.spells[931][gearswap.language] ..
                             '" <me>')
            return
        end
    end
    if gs_skillup.use_item and spell.type ~= "Item" and not buffactive[251] then
        for i, v in ipairs(gs_skill.skill_up_item) do
            if player.inventory[res.items[v][gearswap.language]] then
                cancel_spell()
                send_command(
                    'input /item "' .. res.items[v][gearswap.language] ..
                        '" <me>')
                return
            end
        end
    elseif gs_skillup.use_geo then
        if player.main_job == "GEO" and spell_usable(res.spells[800]) and
            not pet.isvalid then
            if spell.en ~= "Geo-Refresh" then
                cancel_spell()
                send_command(
                    'input /ma "' .. res.spells[800][gearswap.language] ..
                        '" <me>')
                return
            end
            return
        elseif player.sub_job == "GEO" and spell_usable(res.spells[770]) and
            buffactive[541] ~= (gs_skillup.use_trust and 2 or 1) then
            if spell.en ~= "Indi-Refresh" then
                cancel_spell()
                send_command(
                    'input /ma "' .. res.spells[770][gearswap.language] ..
                        '" <me>')
                return
            end
        end
...
function aftercast(spell)
    if pet.isvalid and check_skill_cap() then
        if pet.name == "Luopan" then
            send_command('wait 1.0;input /ja "' ..
                             res.job_abilities[345][gearswap.language] ..
                             '" <me>')
        else
            send_command('wait 1.0;input /ja "' ..
                             res.job_abilities[90][gearswap.language] ..
    ...
    elseif spell.interrupted then
        if spell.en == "Release" then
            send_command('wait 0.5;input /ja "' ..
                             res.job_abilities[90][gearswap.language] ..
                             '" <me>')
            return
        else
    ...
    elseif spell.type == "SummonerPact" then
        local spell_element = (type(spell.element) == 'number' and
                                  res.elements[spell.element][gearswap.language] or
                                  spell.element)
        if spell.en:contains('Spirit') and
            (spell_element == world.weather_element or spell_element ==
                world.day_element) then
            send_command('wait 4.0;input /ja "' ..
                             res.job_abilities[232][gearswap.language] ..
                             '" <me>')
        elseif not spell.en:contains('Spirit') then
            send_command('wait 4.0;input /ja "' ..
                             res.job_abilities[250][gearswap.language] ..
                             '" <me>')
        else
            send_command('wait 3.0;input /ja "' ..
                             res.job_abilities[90][gearswap.language] ..
                             '" <me>')
        end
    ...
    elseif spell.en == "Elemental Siphon" then
        ...
        send_command('wait 3.0;input /ma "' ..
                         gs_skill.skillup_spells[gs_skill.skillup_count] ..
                         '" <me>')
    end
end
function self_command(command)
        ...
        for i, v in ipairs(gs_skill.skillup_table) do
            if v:lower() == commandArgs[2]:lower() then
                ...
                for i, v in pairs(res.spells) do
                    if v.skill == skill_id[gs_skill.skillup_type] and
                        spell_valid(v) and spells_have[v.id] then
                        if #user_settings.user_spells[gs_skill.skillup_type] > 0 then
                            if user_settings.user_spells[gs_skill.skillup_type]:contains(
                                v.name) then
                                gs_skill.skillup_spells:append(
                                    v[gearswap.language])
                            end
                        else
                ...
                if not (#gs_skill.skillup_spells > 0) then
                    add_to_chat(123, "Current Job Can Not Use Spells From " ..
                                    gs_skill.skillup_type)
                    skilluprun = false
                    return
                end
                send_command('input /ma "' ..
    ...
    elseif command == 'changeinstrament' then
        gs_skillup.test_brd = (gs_skillup.test_brd == "Wind" and "String" or
                                  "Wind")
    end
    initialize(window, gs_skillup.box, 'window')
    updatedisplay()
...
function spell_usable(spell)
    if windower.ffxi.get_spells()[spell.id] and
        windower.ffxi.get_spell_recasts()[spell.recast_id] == 0 then
        return true
    end
end
function check_skill_cap()
    if S {'Healing', 'Enhancing', 'Blue', 'Summoning'}:contains(
        gs_skill.skillup_type) then
        if gs_skillup.skill[gs_skill.skillup_type .. ' Magic Capped'] and
            not gs_skillup.test_mode then
            skilluprun = false
            return true
        end
    elseif gs_skill.skillup_type == "Ninjutsu" then
        if gs_skillup.skill[gs_skill.skillup_type .. ' Capped'] and
            not gs_skillup.test_mode then
            skilluprun = false
            return true
        end
    elseif gs_skill.skillup_type == "Geomancy" then
        if gs_skillup.skill['Geomancy Capped'] and
            gs_skillup.skill['Handbell Capped'] and not gs_skillup.test_mode then
            skilluprun = false
            return true
        end
    elseif gs_skill.skillup_type == "Singing" then
        if gs_skillup.skill['Singing Capped'] and
            gs_skillup.skill['Stringed Instrument Capped'] and
            gs_skillup.skill['Wind Instrument Capped'] and
            not gs_skillup.test_mode then
            skilluprun = false
            return true
        end
...
function spell_valid(tab)
    if (tab.levels[player.main_job_id] and tab.levels[player.main_job_id] <=
        player.main_job_level or tab.levels[player.sub_job_id] and
        tab.levels[player.sub_job_id] <= player.main_job_level) and
        tab.targets:contains('Self') and not tab.en:wmatch(
        'Teleport-*|Warp*|Tractor*|Retrace|Escape|Geo-*|Sacrifice|Odin|Alexander|Recall-*') then
        return true
    end
end
...
function initialize(text, settings, a)
    if a == 'window' then
        ...
        if gs_skill.skillup_type == 'Singing' then
            properties:append(
                '\\crCurrent Singing Skill LVL:\n   ${skillssing|0}')
            properties:append(
                '\\crCurrent String Skill LVL:\n   ${skillstring|0}')
            properties:append('\\crCurrent Wind Skill LVL:\n   ${skillwind|0}')
        elseif gs_skill.skillup_type == 'Geomancy' then
            properties:append(
                '\\crCurrent Geomancy Skill LVL:\n   ${skillgeo|0}')
            properties:append(
                '\\crCurrent Handbell Skill LVL:\n   ${skillbell|0}')
        else
    ...
    if a == 'button' then
        ...
        if gs_skillup.test_mode then properties:append('${TESTc}') end
        text:clear()
        text:append(properties:concat('\n'))
    end
end
function updatedisplay()
    ...
    info.start = (skilluprun and '\\cs(0,255,0)Started' or
                     '\\cs(255,0,0)Stopped')
    info.skillssing = (gs_skillup.skill['Singing Capped'] and "Capped" or
                          gs_skillup.skill['Singing Level'])
    info.skillstring = (gs_skillup.skill['Stringed Instrument Capped'] and
                           "Capped" or
                           gs_skillup.skill['Stringed Instrument Level'])
    info.skillwind = (gs_skillup.skill['Wind Instrument Capped'] and "Capped" or
                         gs_skillup.skill['Wind Instrument Level'])
    info.skillgeo = (gs_skillup.skill['Geomancy Capped'] and "Capped" or
                        gs_skillup.skill['Geomancy Level'])
    info.skillbell = (gs_skillup.skill['Handbell Capped'] and "Capped" or
                         gs_skillup.skill['Handbell Level'])
    info.skill = {}
    info.skill.Healing =
        (gs_skillup.skill['Healing Magic Capped'] and "Capped" or
            gs_skillup.skill['Healing Magic Level'])
    info.skill.Enhancing = (gs_skillup.skill['Enhancing Magic Capped'] and
                               "Capped" or
                               gs_skillup.skill['Enhancing Magic Level'])
    info.skill.Summoning = gs_skillup.skill['Summoning Magic Capped'] and
                               "Capped" or
                               gs_skillup.skill['Summoning Magic Level']
    info.skill.Ninjutsu = (gs_skillup.skill['Ninjutsu Capped'] and "Capped" or
                              gs_skillup.skill['Ninjutsu Level'])
    info.skill.Blue = (gs_skillup.skill['Blue Magic Capped'] and "Capped" or
                          gs_skillup.skill['Blue Magic Level'])
    ...
    info.GEOc = (gs_skillup.color.GEO and 'Start GEO' or
                    '\\cs(255,0,0)Start GEO\\cr')
    info.HELc = (gs_skillup.color.HEL and 'Start Healing' or
                    '\\cs(255,0,0)Start Healing\\cr')
    info.ENHc = (gs_skillup.color.ENH and 'Start Enhancing' or
                    '\\cs(255,0,0)Start Enhancing\\cr')
    info.NINc = (gs_skillup.color.NIN and 'Start Ninjutsu' or
                    '\\cs(255,0,0)Start Ninjutsu\\cr')
    info.SINc = (gs_skillup.color.SIN and 'Start Singing' or
                    '\\cs(255,0,0)Start Singing\\cr')
    info.BLUc = (gs_skillup.color.BLU and 'Start Blue Magic' or
                    '\\cs(255,0,0)Start Blue Magic\\cr')
    info.SMNc = (gs_skillup.color.SMN and 'Start Summoning Magic  ' or
                    '\\cs(255,0,0)Start Summoning Magic\\cr  ')
    info.STOPc = (gs_skillup.color.STOP and 'Stop Skillups' or
                     '\\cs(255,0,0)Stop Skillups\\cr')
    info.DOWNc = (gs_skillup.color.DOWN and 'Shutdown After Skillup' or
                     '\\cs(255,0,0)Shutdown After Skillup\\cr')
    info.LOGc = (gs_skillup.color.LOG and 'Logoff After Skillup' or
                    '\\cs(255,0,0)Logoff After Skillup\\cr')
    info.TRUSTc = (gs_skillup.color.TRUST and 'Use Moogle Trust' or
                      '\\cs(255,0,0)Use Moogle Trust\\cr')
    info.REFc = (gs_skillup.color.REF and "Use Geo's Refresh" or
                    "\\cs(255,0,0)Use Geo's Refresh\\cr")
    info.ITEMc = (gs_skillup.color.ITEM and 'Use Skill Up Item' or
                     '\\cs(255,0,0)Use Skill Up Item\\cr')
    info.TESTc = (gs_skillup.color.TEST and 'Change Bard Item' or
                     '\\cs(255,0,0)Change Bard Item\\cr')
...
function file_write()
    if not windower.dir_exists(lua_base_path .. 'data/' .. player.name ..
                                   '/Saves') then
        windower.create_dir(lua_base_path .. 'data/' .. player.name .. '/Saves')
    end
    local file = io.open(lua_base_path .. 'data/' .. player.name ..
                             '/Saves/skillup_data.lua', "w")
    file:write('gs_skillup.box.pos.x = ' .. tostring(gs_skillup.box.pos.x) ..
...
windower.raw_register_event('action message',
                            function(actor_id, target_id, actor_index,
                                     target_index, message_id, param_1, param_2,
                                     param_3)
...
windower.raw_register_event('prerender', function()
    if frame_count % 30 == 0 and window:visible() then updatedisplay() end
