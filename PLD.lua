...
function init_gear_sets()
    ...
    sets.precast.WS['Chant du Cygne'].Acc =
        set_combine(sets.precast.WS.Acc, {waist = "Zoran's Belt"})
...
function job_update(cmdParams, eventArgs) update_defense_mode() end
...
function customize_idle_set(idleSet)
    ...
    if state.Buff.Doom then idleSet = set_combine(idleSet, sets.buff.Doom) end
...
function customize_melee_set(meleeSet)
    if state.Buff.Doom then meleeSet = set_combine(meleeSet, sets.buff.Doom) end
...
function customize_defense_set(defenseSet)
    ...
    if state.EquipShield.value == true then
        defenseSet = set_combine(defenseSet,
                                 sets[state.DefenseMode.current .. 'Shield'])
    end
...
function update_defense_mode()
    if player.equipment.main == 'Kheshig Blade' and
        not classes.CustomDefenseGroups:contains('Kheshig Blade') then
